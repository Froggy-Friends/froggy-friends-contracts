...
contract RibbitItem is Context, ERC165, IERC1155, IERC1155MetadataURI, Ownable {
    ...
    constructor(
        ...
        address _ribbitAddress,
        address _froggyAddress
    ) {
        name = _name;
        symbol = _symbol;
        baseUrl = _baseUrl;
    ...
    function bundleBuy(uint256[] memory ids, uint256[] memory amount) public {
        ...
        for (uint256 i; i < ids.length; i++) {
            ...
            require(
                mintLimitCounter[msg.sender][ids[i]] + amount[i] <= walletLimit[ids[i]],
                "Ribbit item wallet limit exceeded"
            );
